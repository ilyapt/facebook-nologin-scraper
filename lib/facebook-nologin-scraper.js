...
function processPublicPage(doc) {
    ...
    doc('.userContentWrapper').map((o, post)=>{
        ...
        let postData = entities.decode(post.find('.userContent').html());
        ...
        if(!postData){
            const sharedLinkContainer = post.find('a[href*="l.php?u"] div.accessible_elem');
            if(sharedLinkContainer){
                postData = sharedLinkContainer.text();
                postData += ' ' + sharedLinkContainer.parent().attr('href');
            }
        }
